...
<%
   ...
   request.setAttribute("deposite",deposite);
        ...
        String ruc=Constant.RETAIL_TIPO_DOC_RUC;
...
function IngresarCPUF(){
        window.open('OrderAction.do?metodo=insertCPUF','popupCPUF','width=1000,height=500,scrollbars=yes,resizable=yes,modal=yes');
}


 function verDocumentosAGenerar(){
    window.open('OrderAction.do?metodo=verDocumentos','popupDetalle','width=500,height=300,scrollbars=yes,resizable=yes,modal=yes');
...
function cargarTipoDocumentacion()
{
    ...
    var solicitud = "<%=(String)request.getSession().getAttribute("numSol")%>";
    ...
    if(tex === "Automático")
    {
      ...
      if(document.getElementsByName('txtNameKitArrayFinal[]').length>0){
            var totalElementosKit = document.getElementsByName('txtNameKitArrayFinal[]').length;        
            if(solicitud != 'null' && totalElementosKit>0){
                document.getElementById('txtOrderNumber').value = solicitud;
            }
      }
...
{    
    ...
    if(categoria==3 || categoria == 6){       
       ...
       document.getElementById("tdprovincia").style.display="none";
       document.getElementById("tddistrito").style.display="none";
    }else{
       document.getElementById("tdporta").style.display="none";       
       document.getElementById("tdregion").style.display="block";   
       document.getElementById("tdprovincia").style.display="block";  
       document.getElementById("tddistrito").style.display="block";  
    }
...
/* INICIO
 * CREATED BY: VFCONS -- PROYECTO: PM0010913
*/
function getValidatePhone(division){

var strUrlWithParams = "<%=request.getContextPath()%>/OrderAction.do?metodo=getValidatePhone&division="+division;
  var response="";
  $.ajax({  
       type: 'GET',
       ...
       success:function(data){          
            response=data.npparametervalue1+"|"+data.npparametervalue2;
       },
...
function validPhoneNumber(phoneNumber,pattern,longitud){
    if (phoneNumber.length==longitud){
        var digit = phoneNumber.charAt(0);
    ...
    return "debe poseer "+ longitud + " dígitos";

}
/*function getPhoneNumber(xml){
     var portabilidad = xml.getElementsByTagName("portabilidad")[0];
     var data = portabilidad.getElementsByTagName("data");
        ...
        document.getElementById("tmpPhoneNumber").value = numero;
}*/
function getPhoneNumber(imei){
    var phone = "";
    var strUrlWithParams = "<%=request.getContextPath()%>/OrderAction.do?metodo=PortabilidadView&imei="+imei;
  $.ajax({  
       type: 'GET',
...
{   
    ...
    /*fin mespinoza*/
        ...
        /** VFCONS - PM0010913 -INICIO **/  
        
                     
    var selec = document.getElementById('cmbDivision');
    var option = selec.options[selec.selectedIndex];
    
    var sel = document.getElementById('cmbCategoria');
    var opt = sel.options[sel.selectedIndex];
    
    var validatePhone=0;
    var pattern = "";
                    
    if(opt.text.indexOf("Portabilidad")!=-1) {
                        
         if(option.text.indexOf("Fija")!=-1 || option.text.indexOf("Fijo")!=-1) {
            //consulta a bd si existe validación - devuelve regex
            pattern = getValidatePhone(1);
            validatePhone=1;
               
        }else{
        
            //consulta a bd si existe validación - devuelve regex
            pattern = getValidatePhone(2);
            validatePhone=2;
        }
        
    }
    if(validatePhone!=0 && pattern!=""){
        ...
        for(var i=0; i < totalElementos; i++){
                ...
                req.onreadystatechange = getReadyStateHandler(req,getPhoneNumber);
                req.open("GET","/retail_pos-retail_pos-context-root/OrderAction.do?metodo=PortabilidadView&imei=" + imei,false);
                req.setRequestHeader("Content-Type","application/ISO-8859-1");
                req.send(null);  */ 
                
                //obt campo temporal (seteado en la funcion getphonenumber()
                var phoneNumber =  getPhoneNumber(imei); // document.getElementById("tmpPhoneNumber").value;
                var longitud = (validatePhone ==1)? 8 : 9;
                var flagPhone = validPhoneNumber(phoneNumber,pattern,longitud);
                if(flagPhone!=""){
                    var tmpMsg= (validatePhone ==1 ? "teléfono fijo" : "´teléfono Móvil");
                    alert("El número de "+ tmpMsg +" a portar, " + flagPhone);
                    return false;
                }
        }
    }
    ...
    var validRadio    =  document.forms[0].validRadio.value;
    ...
    var totalAnticipado = document.forms[0].txtPagoAnticipadoTotal.value;
     ...
     if (!$('input:radio[name=pagoAnticipado]').is(':checked') && tipoDocumento!='RUC'&& validRadio > 0 ) {
    ...
    req.open("GET","<%=request.getContextPath() %>/OrderAction.do?metodo=ValidarTimeLimitFromJsp&cmbSubCategoria=" + subCategoria+"&tipoValidacionHora=CREACION",true);
    req.setRequestHeader("Content-Type","application/ISO-8859-1");
    req.send(null);   
}
/*mespinoza*/
function verifyPhoneEvaluation(categoryId){
	...
	$.ajax({
		type: 'POST',
           	async:false, 
           	contentType: "text/json,ISO-8859-1",
           	url: '<%=request.getContextPath() %>/OrderAction.do?metodo=verifyPhonePorta&imei=' + strImei+'&userLogin='+userLogin+'&categoryId='+categoryId,
           	data: null,
           	success:function(data){
           		//alert('ESTOY DENTRO');
...
{
    window.open("OrderAction.do?metodo=orderPrint","","width=415,height=300,scrollbars=auto,modal=yes");        
}
...
function fBotonPopup(fOrdenId,fRespuestaReniecIdData,fBotonRespuesta)
{
    ...
    req.open("GET","<%=request.getContextPath() %>/OrderAction.do?metodo=ValidarReniec&sOrdenId=" +fOrdenId+"&sRespuestaReniecIdData="+fRespuestaReniecIdData+"&sBotonRespuesta="+fBotonRespuesta+"&sMotivorechazo="+cmbMotivrechazo,true);
    req.setRequestHeader("Content-Type","application/ISO-8859-1");
    req.send(null);
...
function ValidarReniec(resultado){
    if(DatosReniec()==true)
    {
      ...
      document.forms[0].action="<%=request.getContextPath() %>/OrderAction.do?metodo=ValidarReniec";    
      document.forms[0].submit();
    }
}
...
function updatetimeLimitExceeded(miXml){
		...
		var flagTimeLimitExceeded = LimitExceeded.getElementsByTagName("timeLimitExceeded")[0].childNodes[0].nodeValue;		
                if (flagTimeLimitExceeded=="true"){                  
                    ...
                    window.open('OrderAction.do?metodo=orderDetail&cmbSoluction='+cmbSoluction+"&cmbLineaProducto="+cmbLineaProducto,'popupDetalle','width=1000,height=500,scrollbars=yes,resizable=yes,modal=yes');
...
{               
    ...
    req.open("GET","<%=request.getContextPath() %>/OrderAction.do?metodo=cargarCategoria&cmbDivision=" + division + "&pageOrig=" + pageOrig,true);
    req.setRequestHeader("Content-Type","application/ISO-8859-1");
    req.send(null);
...
function cargarSubCategoria()
{
    ...
    req.open("GET","<%=request.getContextPath() %>/OrderAction.do?metodo=cargarSubCategoria&cmbCategoria=" + categoria,true);
    req.setRequestHeader("Content-Type","application/ISO-8859-1");
    req.send(null);
...
function cargarSolucion()
{
    ...
    req.open("GET","<%=request.getContextPath() %>/OrderAction.do?metodo=cargarSolucion&cmbSubCategoria=" + subCategoria,true);
    req.setRequestHeader("Content-Type","application/ISO-8859-1");
    req.send(null);
    ...
    if(subCategoria!="0"){
        getMostrarVentaAnticipada(subCategoria,tipoDocumento);
   }     
}

  function getMostrarVentaAnticipada(subCategoria,tipoDocumento){
  var strUrlWithParams = "<%=request.getContextPath()%>/OrderAction.do?metodo=getMostrarVentaAnticipada&subCategoria="+subCategoria+"&tipoDocumento="+tipoDocumento;
  var response="";
      $.ajax({  
           type: 'POST',
           contentType: "text/json,ISO-8859-1",
           ...
           data:  null,
           success:function(data){          
            if(data=="1"){ document.getElementById('tdskuAnticipado').style.display='inline';
                           document.getElementById('tdPagoAnticipado').style.display='inline';
                           document.getElementById('trAnticipado').style.display='inline';
                         }else {
                          document.getElementById('tdskuAnticipado').style.display='none';
                           document.getElementById('tdPagoAnticipado').style.display='none';
                           document.getElementById('trAnticipado').style.display='none';
                         }            
...
function cargarLineaProducto()
{
    ...
    req.open("GET","<%=request.getContextPath() %>/OrderAction.do?metodo=cargarLineaProducto&cmbSoluction=" + cmbSoluction,true);
    req.setRequestHeader("Content-Type","application/ISO-8859-1");
    req.send(null);
}
...
function newXMLHttpRequest(){
 ...
 function viewPopupOrdenesPrevias(){
    var url = '<%=request.getContextPath() %>/OrderAction.do?metodo=viewWindowPOrders';   
 ...
 function getStatusViewOrdersPrevious(){
    ...
    req.open("GET",'<%=request.getContextPath() %>/OrderAction.do?metodo=getStatusPreviousOrder&division='+divisionid+'&categoria='+categoriaid+'&subcategoria='+subcategoriaid,true);
    req.setRequestHeader("Content-Type","application/ISO-8859-1");
    req.send(null);
 }
 ...
 function getListPreviousOrders(){
    ...
    req.open("GET",'<%=request.getContextPath() %>/OrderAction.do?metodo=listPreviousOrders&customer='+customerretailid+'&division=' + divisionid+'&categoria='+categoriaid+'&subcategoria='+subcategoriaid,true);
    req.setRequestHeader("Content-Type","application/ISO-8859-1");
    req.send(null);
 }
...
<html:hidden property="validRadio" />
...
<table width="100%" border="0">
 ...
 <table width="100%"  border="0">
  ...
  <table width="100%"  border="0">
    <tr class="ListRow1">
      <td width="13%" class="LeftSubTab"><strong>Nro de Solicitud</strong></td>
      <td width="87%"><html:text property="txtOrderNumber" styleId="txtOrderNumber" maxlength="14" size="20" disabled="${statusNroSolicitud}"  onkeypress="alfanumerico2(this);"/></td>
    </tr>
  </table>
  <table width="100%"  border="0">
    <tr class="ListRow1">
      <td width="13%" class="LeftSubTab"><strong>División</strong></td>
      <td width="18%">
      ...
      <td width="12%" class="LeftSubTab"><strong>Categoría</strong></td>
      <td width="17%">
  ...
  <table width="100%"  border="0">
  <tr class="ListRow1">
      <td width="13%" class="LeftSubTab"><strong>Soluci&oacute;n</strong></td>
      <td width="18%">
      ...
      <td width="12%" class="LeftSubTab"><strong>Línea de Producto</strong>
                      </td>
      <td width="17%">
      ...
      <td width="68%" class="hide" id="wrapperCentroCosto">
                          <html:select property="cmbSite"  styleId="cmbSite"></html:select>
      </td>
	  </tr>  
      </table>
      </td>
    ...
    <table width="100%" id="tablesdd" border="0" style="<%=request.getSession().getAttribute("flag")%>">
      <tr class="ListRow1">
        <!-- Aumento de nuevos campos para una orden -->
          <td width="13%" class="LeftSubTab" id="divTipoDocumentacion" style="${statussdd}"><strong>Tipo Documentación</strong></td>
          <td width="18%" id="divValTipoDocumentacion" style="${statussdd}"> 
          ...
          <td width="12%" class="LeftSubTab" id="divEmail" style="<%=request.getSession().getAttribute("automatica")%>"><strong>Email</strong></td>
          <td width="17%" id="divTxtEmail" style="<%=request.getSession().getAttribute("automatica")%>"><html:text property="txtCustomerEmail" styleId="txtCustomerEmail" maxlength="50" size="30"/>
          </td>
          <td width="12%" class="LeftSubTab" id="divMotivo" style="<%=request.getSession().getAttribute("manual")%>"><strong>Motivo</strong></td>
    
          <td width="17%" id="divTxtMotivo" style="<%=request.getSession().getAttribute("manual")%>">
          ...
          <!-- Fin de cambio de campos nuevos -->
	  </tr>  
      </table>
    
    <div id="div_cpuf">

  <table width="100%"  border="0" style="<%=request.getSession().getAttribute("flag")%>">
...
<!-- AMATEO -->
    ...
    <td height="30" colspan="8">
      ...
      <input type="button" value="Agregar Detalle" style="width:100px" id="btnOpenAddItem" onClick="javascript:AgregarDetalleOrden(document.forms[0].cmbSoluction.value,document.forms[0].cmbLineaProducto.value);" >
    ...
    <tr id="trAnticipado"  style="<%=request.getSession().getAttribute("flagTdMontoAnticipado")%>"  class="ListRow1">
      <td width="20%" style="font-size:9px" class="LeftSubTab"><strong>Aplicar Pago Acticipado</strong></td>      
      ...
      <td width="10%"></td>
      <td width="10%"></td>
      <td width="20%" style="font-size:9px" class="LeftSubTab"><strong>Total Pago Anticipado:</strong></td>
  ...
  <!--EQUIPOS -->
    ...
    <tr class="ListRow1">
      ...
      <td width="10%" class="LeftSubTab"><div align="center"><strong>SKU</strong></div></td>
      <td width="10%" class="LeftSubTab"><div align="center"><strong>Precio</strong></div></td>
      ...
      <td width="7%" class="LeftSubTab"  id="tdPagoAnticipado"  style="<%=request.getSession().getAttribute("flagTdMontoAnticipado")%>" ><div align="center"><strong>Pago <br/> Anticipado</strong></div></td>
      

      <td width="11%" class="LeftSubTab"><div align="center"><strong>Modelo</strong></div></td>
      ...
      <td class="LeftSubTab" id="tdporta"  style="display:none" ><div align="center"><strong>Portabilidad</strong></div></td>
      <td class="LeftSubTab" id="tdregion" style="display:none" ><div align="center"><strong>Regi&oacute;n</strong></div></td> 
       <td class="LeftSubTab" id="tdprovincia" style="display:none" ><div align="center"><strong>Provincias</strong></div></td> 
       <td class="LeftSubTab" id="tddistrito" style="display:none" ><div align="center"><strong>Distritos</strong></div></td> 
      </tr>
  <logic:notEmpty name="OrderForm" property="listaOrdenes">
      ...
      <td class="ListRow1"><div align="center" id="ocultar_imagen" style="<%=request.getAttribute("statusAgregarDetalle")%>"><input name='txtIdKitArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="idKit"/>"><a href='javascript:eliminarDetalleFinal(<%=idItem%>);'><img src='images/Eliminar.gif' width='16' height='15' title='Eliminar' border='0'></a></div></td>
      <td class="ListRow1"><div align="center"><input name='txtNameKitArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="nameKit"/>"><bean:write name="detailFinal" property="nameKit"/></div></td>
      <td class="ListRow1"><div align="center"><input name='txtSkuKitArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="skuKit"/>"><bean:write name="detailFinal" property="skuKit"/></div></td>
      <td class="ListRow1">
        <input name='txtCurrencyArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="currency"/>">
        <div align="right">
            <input name='txtCostKitArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="cost"/>">
            <bean:write name="detailFinal" property="cost"/>
        </div>
        <input name='txtSolutionCodeFinal[]' type='hidden' value="<bean:write name="detailFinal" property="codeSolution"/>">   
        <input name='txtProductLineFinal[]' type='hidden' value="<bean:write name="detailFinal" property="productLineId"/>">
     </td>      
      <%}else{%>
      ...
      <td class="ListRow1">
        <input name='txtIdPlanArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="idPlan"/>">
        <div align="center"><input name='txtPlanNameArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="namePlan"/>"><bean:write name="detailFinal" property="namePlan"/></div>
        <input name='txtIdProductArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="idProduct"/>">        
       
        <input name='txtEstadoAnticipadoArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="estadoAnticipado"/>">
        <input name='txtMontoAnticipadoThinkisArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="montoAnticipadoThinkis"/>"> 
        <input name='txtDescripcionErrorArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="messageErrorThinkis"/>">         
        <input name='txtBillcycleOrigenArrayFinal[]' type='hidden'  value="<bean:write name="detailFinal"  property="cicloOrigen"/>"> 
        <input name='txtBillcycleDestinoArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="cicloDestino"/>"> 
        <input name='txtCustomerIDArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="idCustomer"/>"> 
      </td>       
      <td  class="ListRow1"    style="<%=request.getSession().getAttribute("flagTdMontoAnticipado")%>" ><div align="center"><input name='txtSkuAnticipadoArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="skuAnticipado"/>"><bean:write name="detailFinal" property="skuAnticipado"/></div></td>
      <td  class="ListRow1"    style="<%=request.getSession().getAttribute("flagTdMontoAnticipado")%>" ><div align="center"><input name='txtMontoAnticipadoArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="montoAnticipado"/>"><bean:write name="detailFinal" property="montoAnticipado"/></div></td> 
      <td class="ListRow1"><div align="center"><input name='txtModelNameArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="nameModel"/>"><bean:write name="detailFinal" property="nameModel"/></div></td>
      <td class="ListRow1"><div align="center"><input name='txtIMEIArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="imei"/>"/><bean:write name="detailFinal" property="imei"/></div></td>
      <td class="ListRow1"><div align="center"><input name='txtSIMArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="sim"/>"/><bean:write name="detailFinal" property="sim"/></div></td>      
     <%
     ...
     { %>
       <td class="ListRow1"> <a id="idportabilidad" name="idportabilidad" data="<bean:write name="detailFinal" property="idKit"/>" class="popup_view" value="aaaa"  href="#" title="Iniciar Sesi&oacute;n">Portabilidad</a></td>
           <% }else{ %>  
           <td class="ListRow1"><div align="center"><input name='txtNombresArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="nombreRegionSeleccion"/>"><bean:write name="detailFinal" property="nombreRegionSeleccion"/></div></td>
      <td class="ListRow1"><div align="center"><input name='txtNombresProvinciaArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="nombreProvinciaSeleccion"/>"/><bean:write name="detailFinal" property="nombreProvinciaSeleccion"/></div></td>
      <td class="ListRow1"><div align="center"><input name='txtNombresDistritoArrayFinal[]' type='hidden' value="<bean:write name="detailFinal" property="nombreDistritoSeleccion"/>"><bean:write name="detailFinal" property="nombreDistritoSeleccion"/></div></td>
           <% } %>  
      </tr>
  ...
  var phone='<%=request.getAttribute("PHONEREPETIDO").toString()%>';
  alert('el numero: '+phone+' a portar ya fue agregado a otro Producto de la grilla');
  </script>
  </logic:notEmpty>
                        ...
                        <table  align="left">
                            <tr>
                                <td class="LeftSubTab">
                                        ...
                                        <img src="OrderAction.do?metodo=imageGetter&pTypeImg=foto" height="250" width="250">    
                            ...
                            <tr>
                                <td>
                                        ...
                                        <img src="OrderAction.do?metodo=imageGetter&pTypeImg=firma" height="80" width="250">
...
value='<%=request.getAttribute("resultEval")%>' />
  ...
  <!-- Fin -->       
  <input name='tmpPhoneNumber' id="tmpPhoneNumber" type='hidden'/>
</html:form>
...
<script defer="defer" type="text/javascript" language="javascript">
    ...
    {       
        //SubCategoria
        var flagSubCategoria = "<%=request.getAttribute("statusCmbSubCategoria")%>";
        var pages = "<%=request.getContextPath() %>/OrderAction.do?metodo=cargarSubCategoria&cmbCategoria=" + categoria;
...
$(function(){
    ...
    centroCostos.listarXdivision = function(idDivision){
        ...
        var url = "<%=request.getContextPath() %>/OrderAction.do?metodo=listarCentroCostos";        
    ...
    centroCostos.applySalesRule = function(idDivision,tipoOperacion,resultEval){
        var url = "<%=request.getContextPath() %>/OrderAction.do?metodo=getSalesRule";        
